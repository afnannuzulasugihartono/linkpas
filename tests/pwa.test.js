...
test('HTML links manifest and Apple install metadata', () => {
  ...
  assert.match(html, /launch-probe\.js/);
...
test('service worker caches app shell for offline use', () => {
  ...
  assert.match(sw, /launch-probe\.js/);
...
test('public demo bundle contains PWA and license assets after build', () => {
  for (const file of ['manifest.webmanifest', 'sw.js', 'launch-probe.js', 'license.js', 'icon-192.png', 'icon-512.png', 'apple-touch-icon.png']) {
